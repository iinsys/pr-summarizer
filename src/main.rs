...
async fn main() -> Result<()> {
    ...
    let event_path =
        env::var("GITHUB_EVENT_PATH").context("GITHUB_EVENT_PATH environment variable not set")?;

    let event_payload =
        utils::read_github_event(&event_path).context("Failed to read GitHub event payload")?;

    // Extract PR number and repository from the event
    let (repo_owner, repo_name) =
        utils::get_repository_info().context("Failed to get repository information")?;

    let pr_number = utils::extract_pr_number(&event_payload)
        .context("Failed to extract PR number from event payload")?;

    // Create GitHub client
    let token = env::var("GITHUB_TOKEN").context("GITHUB_TOKEN environment variable not set")?;

    let github_client =
        github::create_github_client(&token).context("Failed to create GitHub client")?;
    ...
    // Generate summary
    let summary = summarizer::generate_summary(&pr_info).context("Failed to generate summary")?;
