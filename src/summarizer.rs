use crate::github::{FileStatus, PrInfo};
use anyhow::Result;
...
fn infer_changes_from_files(files: &[crate::github::ChangedFile]) -> Vec<String> {
    ...
    let mut _file_extensions = files
        .iter()
        .filter_map(|f| {
    ...
    for (dir, dir_files) in files_by_dir {
        ...
        let added = dir_files
            ...
            .count();
        let modified = dir_files
            .iter()
            .filter(|f| matches!(f.status, FileStatus::Modified))
            .count();
        let removed = dir_files
            .iter()
            .filter(|f| matches!(f.status, FileStatus::Removed))
            .count();
